...
```{r}
ct_all <- rbind(swp, wb, ebparks, naut, usgs, prop1) %>%
...
ct_all2 = ct_all %>%
  ...
  mutate(Advisory = case_when(Analyte2 == "Microcystins" & Result > 0.8 & Result < 6 ~ "Caution",
...
ct_2022 <- st_intersection(ct_all_sf, reg3)%>%
