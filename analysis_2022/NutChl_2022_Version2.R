...
# Find the start and end dates for each site in the current dataset
HABs22Nuts_Summy <- hab_nutr_chla_mvi %>%
...
HABs22Nuts_RegionSummy <- nutssf %>%
  group_by(Source, Station, Stratum2) %>%
  summarize(first=min(as.Date(Date)),last=max(as.Date(Date)), maxNO4=max(DissAmmonia),
            maxPO4=max(DissOrthophos))
