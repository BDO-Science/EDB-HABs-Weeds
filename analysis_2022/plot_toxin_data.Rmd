...
(map_cyano <- ggplot()+
  ...
  geom_sf(data =reg3crop, aes(fill = Stratum2), inherit.aes = FALSE, alpha = 0.3)+
...
(map_advisories <- ggplot()+
  ...
  scale_color_manual(values = c("yellow",  "orange"), labels = c("Caution", "Warning"), name = "Advisory Level")+
  theme_bw()+ 
...
(map_advisories_allyears <- ggplot()+
  geom_sf(data = reg3crop, aes(fill = Stratum2), alpha = 0.3) +
  geom_sf(data = WW_Delta, color = "grey", fill = "lightblue", alpha= 0.5)+
  scale_fill_manual(values = reg3$colors, guide = NULL)+
   #scale_shape_manual(values = c(7, 2, 15, 3, 8, 18, 1, 6)) +
    scale_shape_manual(values = c(16, 17, 15, 8, 6, 7, 18, 7)) +
  theme_bw()+ylab("")+xlab("")+
  geom_sf(data = ct_advis_allyears, aes(color = Advisory, shape = Study), size = 4)+
    facet_wrap(~Year) + 
  annotate("text", x = -121.6, y = 37.8, label = "Clifton Court")+
  annotate("text", x = -121.48, y = 37.95, label = "Mildred Island")+
  annotate("text", x = -121.3, y = 37.93, label = "Stockton")+
  annotate("text", x = -121.75, y = 37.99, label = "Big Break")+
  scale_color_manual(values = c("yellow",  "orange", "red"), labels = c("Caution", "Warning", "Danger"), name = "Advisory Level")+
  theme_bw()+ 
    theme(axis.text.x = element_text(angle = 45, hjust = 1),
          axis.title = element_blank()) + 
  annotation_north_arrow(location = "tr", which_north = "true",
...
(map_advisories_month_2022 <- ggplot()+
  ...
  scale_fill_manual(values = reg3$colors, guide = NULL)+
  geom_sf(data = advisories, aes(color = Advisory),  size = 2, inherit.aes = FALSE) +
...
spatt_summer_sf <- filter(spatt_data, Month > 4 & Month <12) %>%
  ...
  ungroup() %>%
  group_by(Analyte) %>%
  mutate(scaledResult = scale(maxResult)) %>%
...
ggplot(spatt_summer_sf) + geom_histogram(aes(maxResult), binwidth = 10)

ggplot(spatt_summer_sf) + geom_tile(aes(Month, Analyte, fill = maxResult)) +
...
spatt_summer_sf$Result <- factor(spatt_summer_sf$Result, levels = c("ND", "<10", "10-100", "100-1000", ">1000"))
spatt_summer_sf$ResultPerDay <- factor(spatt_summer_sf$ResultPerDay, levels = c("ND", "<0.1", "0.1-1", "1-10", ">10"))

(map_spatt <- ggplot() + 
    geom_sf(data = WW_Delta, color = "lightcyan1", fill = "lightcyan1", alpha = 0.9) +
  geom_sf(data = spatt_summer_sf, aes(color = Result), inherit.aes = FALSE) + facet_grid(Analyte~fMonth)+ 
...
(map_spatt_pd <- ggplot() + 
    geom_sf(data = WW_Delta, color = "lightcyan1", fill = "lightcyan1", alpha = 0.9) +
  geom_sf(data = spatt_summer_sf, aes(color = ResultPerDay), inherit.aes = FALSE) + facet_grid(Analyte~fMonth)+ 
  scale_colour_manual(values = c("gray65", viridis::viridis(5, option = "plasma")[1:4])) +
  scale_x_continuous(limits = c(-121.9, -121.1)) +
  scale_y_continuous(limits = c(37.6, 38.4)) +
...
png(filename = here("analysis_2022", "figures", "map_advisories_2021_2022.png"), width = 9, height = 6, units = "in", pointsize = 12, family = "sans", res = 300)
map_advisories_allyears
dev.off()


png(filename = here("analysis_2022", "figures", "spatt_map_2022.png"), width = 9, height = 7.5, units = "in", pointsize = 10, family = "sans", res = 300)
map_spatt
dev.off()
...
png(filename = here("analysis_2022", "figures", "map_advisories_2022_month.png"), width = 8, height = 7, units = "in", pointsize = 12, family = "sans", res = 300)
