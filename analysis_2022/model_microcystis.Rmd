...
data_sumfall <- filter(mic_data, Season != "Spring")
m2 <- glm(MicroPA ~fYear + Region + fYear:Region , family=binomial(link='logit'),data=data_sumfall)
summary(m2)
anova(m2)
...
plot(data_sumfall$Season, resid_m2)
```
...
fit.emm <- emmeans(m2, "fYear", data=mic_data)
resultsyear <- pairs(fit.emm, adjust="tukey")
plot(fit.emm, comparisons = TRUE)
model_microcystis_year_results <- as.data.frame(resultsyear) %>%
  mutate(across(.cols = c(estimate:SE, z.ratio), ~round(.x, digits = 3))) 

fit.emm2 <- emmeans(m2, "Region", data=mic_data)
