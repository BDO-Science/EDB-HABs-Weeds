...
sta_cont <- read_excel(here(data_root, "data_clean", "continuous_stations.xlsx")) %>%
  ...
  mutate(Source = case_when(
    Station == "FRK" ~ "DWR_EMP",
    Station %in% c("MDM", "ORQ", "DSJ", "SJG", "VNS") ~ "USGS_CAWSC",
    TRUE ~ "DWR_NCRO"
  ))
...
```{r}
sta_noSJ <- sta_sf %>%
  filter(!Station %in% c("SJC", "SJG", "VNS"))

(wq_map <- ggplot() + 
  geom_sf(data = delta_4326, fill = "lightblue", color = "lightblue") +
  geom_sf(data = reg3, aes(fill = Stratum2), inherit.aes = FALSE, alpha = 0.3)+
  geom_sf(data = sta_noSJ, aes(color = WQ, shape = Source, size = WQ), inherit.aes = FALSE)+
  geom_text(data = sta_noSJ %>% filter(WQ == "Continuous"), aes(x = Longitude, y = Latitude, label =  Station), nudge_x = c(-0.02, -0.005, 0.05, 0.05, 0.02, -0.005, -0.005, -0.005, -0.02, -0.02), nudge_y = c(0.02,-0.01, 0.01, 0.01, 0.03, 0, 0, -0.01,0), size = 3, vjust = 1, hjust = 1)+
  scale_x_continuous(limits = c(-121.9, -121.25)) +
  scale_y_continuous( limits = c(37.75, 38.57))+
  scale_fill_manual(values = reg3crop$colors, name = NULL, guide = NULL)+
