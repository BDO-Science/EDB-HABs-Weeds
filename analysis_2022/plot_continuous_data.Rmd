...
wq_data2 <- wq_data %>%
  ...
  mutate(TUCPComp = case_when(WY %in% c(2013, 2020)~ "DroughtNoTUCP", 
                            WY %in% c(2014, 2015, 2016, 2021)~ "DroughtTUCP", 
                            WY == 2022 ~ "2022",
                            #Year %in% c(2013, 2016, 2018, 2020) ~ "DryNoTUCP",
...
wq_data_summary_TUCP <- wq_data2 %>%
  group_by(TUCPComp, Region, Analyte, Date2) %>%
  summarize(mean = mean(dailyval))

wq_data_summary_EDB <- wq_data2 %>%
  group_by(EDBComp, Region, Analyte, Date2) %>%
  summarize(mean = mean(dailyval))
...
spc <- readRDS(here(data_root, "data_raw", "CDEC_spc_EMM_TMS_2013_2022.rds")) %>%
  mutate(Date2 = paste0(1980, "-", Month, "-", day(Date)),
         Date2 = ymd(Date2))
spc_dry <- filter(spc, Year %in% c(2013, 2016, 2018, 2020, 2022),
