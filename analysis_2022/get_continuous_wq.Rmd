...
f_get_NWIS <- function(siteNumbers=11312676, parameterCd = c('00010','63680', '00095', '00400', '00300', '32316'), startDate = "2022-01-01", endDate = "2022-10-01", tz = "Etc/GMT+8"){
  ...
  # write out
  readr::write_csv(data2, paste0("analysis_2022/data_raw/USGS_NWIS_", siteNumbers, "_WQ.csv"))
...
DSJ: Latitude 38°00'49",   Longitude 121°40'00"   NAD27
...
f_get_NWIS(siteNumbers = 11313433)
...
DSJ <- read_csv(here(data_root, "USGS_NWIS_11313433_WQ.csv"))%>%
  rename(Datetime = date_time, 
         wtemp = wtemp_inst,
         spc = spec_cond_inst,
         turbidity = ts213_ysi_exo_turb_inst)%>%
...
```{r}
stalatlon <- read_excel("data/continuous stations.xlsx") %>%
  dplyr::select(-Type)

unique(wq_all$Site)

wq_all <- readRDS(here(data_root, "../data_clean", "continuous_wq_2015_2022.rds")) %>%
  left_join(stalatlon %>% rename(Site = StationCode))
wq_2022 <- readRDS(here(data_root, "../data_clean", "continuous_wq_2022_allvals.rds"))%>%
  left_join(stalatlon %>% rename(station = StationCode))
```
