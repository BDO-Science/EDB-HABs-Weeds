...
(map_cyano <- ggplot()+
  ...
  scale_shape_manual(values = c(18, 2, 15, 3, 7, 8, 1, 6)) +
  theme_bw()+
...
### Filter only Microcystins and Anatoxins and Cylindrospermopsins
```{r}
...
(plot_data_regions <- ggplot(ct_all_zeros, aes(x = Date, y = Result, color = nonzero)) + 
  ...
  facet_grid(Analyte~Stratum2, scales = "free_y",
             labeller = as_labeller(c(Anabaenopeptins = "Anabaenopeptins", Anatoxins = "Anatoxins", Cylindrospermopsins = "Cylindrospermopsins", Saxitoxin = "Saxitoxins", `Microcystins/Nodularins` = "Microcystis/\nNodularins",
...
## Non-zeros - used this one
```{r}
library(ggnewscale)

# Rename Microcystins/Nodularins as Microcystins
nonzero <- ct_all %>% filter(!is.na(Stratum2)) %>%
  mutate(Analyte = case_when(grepl("Microcystins/Nodularins", Analyte) ~ "Microcystins",
                             TRUE~ Analyte)) %>%
  filter(Analyte %in% c("Microcystins", "Anabaenopeptins", "Anatoxins", "Cylindrospermopsins", "Saxitoxins")) %>%
  mutate(nonzero = ifelse(Result == 0, "zero", "nonzero"))
unique(nonzero$Analyte)

(plot_advisories_by_region2 <- ggplot(nonzero, aes(x = Date, y = Result, color = nonzero)) + geom_point(aes(shape = Study))+
    scale_color_manual(values = c("black", "gray")) +
    guides(color = "none") + 
    new_scale_colour() +
  geom_hline(data = filter(health, AdviseType != "Danger \nTier III"), aes(yintercept = Advisory, color = AdviseType))+
...
## Map of 2022 cyanotoxin data only
```{r}
advisories <- mic_only %>%
...
png(filename = here("analysis_2022", "figures", "cyano_by_region.png"), width = 8.5, height = 8, units = "in", pointsize = 12, family = "sans", res = 300)
plot_data_regions
dev.off()
