...
# Add TUCP start and end dates into dataset, organize by location
wq_data2 <- wq_data %>%
...
daily_frk0 <- wq_frk %>%
  filter(fYear %in% c("2014", "2015", "2020", "2021", "2022")) %>%
...
wq_a <- wq_data2 %>% 
  filter(Analyte != "Flow",
         Analyte != "Velocity",
         !Region %in% c("San Joaquin")) %>%
  mutate(dailyvalo = dailyval,
    dailyval = ifelse(Analyte %in% c("Turb", "Chl.a"), log(dailyval+1), dailyval))
...
# All years 
wq_yearcomp <- wq_a %>%
  filter(Year %in% c(2014, 2015, 2016,2020, 2021, 2022))  %>%
  ungroup() %>%
  droplevels()

wq_yearcomp <- subset(wq_yearcomp, Year%in% c(2014, 2015, 2016, 2020, 2021, 2022))


wq_yearcomp %>% group_by(Analyte, Year) %>%
  summarize(maxo = max(dailyvalo))


wq_EDB <- wq_data_summary_EDB %>%
  filter(!Region %in% c("Liberty Island"),
...
```{r}
log(10 + 1)
templine <- data.frame(Analyte = c("Temp", "Chl.a"), int = c(19, 2.397895))
...
(plot_cont_2022 <- ggplot() +
    ...
    facet_wrap(~Analyte, scales = "free_y", nrow = 6, strip.position = "left", 
             labeller = as_labeller(c(Chl.a = "log Chl a\n(µg/L)",DO.Conc = "DO \n(mg/L)", 
            #Flow = "Flow (cfs)", 
    pH = "pH",SpCond="SpCnd \n(mS/cm)", Turb="log \nTurbidity \n(FNU)", Temp="Water Temp \n(°C)")))+
...
### Across years
```{r}
analytes <- data.frame(Analyte = rep(unique(wq_yearcomp$Analyte),6),
                       fYear = c(rep("2014", 6), rep("2015", 6), rep("2016", 6), rep("2020", 6), rep("2021", 6), rep("2022", 6)))

tucp.df <- tucp_dates %>%
  mutate(fYear = factor(WY)) %>%
  filter(fYear %in% c("2014", "2015", "2016", "2020", "2021", "2022")) %>%
...
(plot_cont_jday <- ggplot() +
  ...
  facet_grid(Analyte~fYear, scales = "free_y",  labeller = as_labeller(c(`2014` = "2014 (T)",
...
png(filename = here("analysis_2022", "figures", "plot_wq_years.png"), width = 9.5, height = 8.5, units = "in", pointsize = 12, family = "sans", res = 300)
plot_cont_jday
dev.off()
```
...
### Frk vs MI - EDB - Did not use this one
```{r}
