...
spatt_summer_sf <- filter(spatt_data, Month > 4 & Month <12) %>%
  ...
  summarize(sumResult = sum(Result)) %>%
  ungroup() %>%
  group_by(Site_Name, Analyte) %>%
  mutate(scaledResult = scale(sumResult)) %>%
  mutate(Result = case_when(sumResult == 0 ~ "ND",
                              sumResult < 10 ~ "<10",
                               sumResult >= 10 & sumResult < 100 ~ "10-100",
                               sumResult >= 100 & sumResult < 1000 ~ "100-1000",
                               sumResult >= 1000 ~ ">1000"))

ggplot(spatt_summer_sf) + geom_histogram(aes(sumResult), binwidth = 10)

ggplot(spatt_summer_sf) + geom_tile(aes(Month, Analyte, fill = sumResult)) +
