...
df_ncro_2022_c <- df_ncro %>%
  ...
  mutate(
    ...
    Station = case_when(
      ...
      str_detect(StationName, "^Rock Slough @ Contra Costa WD Fish Screen") ~ "NA_RSL",
      str_detect(StationName, "^Rock Slough at Delta Road Bridge") ~ "NA_INB",
...
mutate(
  ...
  left_join(df_ncro_coord_c, by = "Station") %>%
   mutate(Longitude = case_when(Station == "MHR" ~ -121.384000,
                              Station == "OBD" ~ -121.543938,
                              Station == "DGL" ~ -121.425000,
                              TRUE~ Longitude),
         Latitude = case_when(Station == "MHR" ~ 37.877000,
                               Station == "OBD" ~ 37.810963,
                              Station == "DGL" ~ 37.815000,	
                              TRUE~ Latitude)) %>%
...
Do not run this one, but think about it for later - join by station number instead of codes (which are for continuous data) - get the correct station number metadata from Tyler.
```{r}
df_ncro_coord_c_alt <- df_ncro_coord %>%
  janitor::clean_names(case = "upper_camel")%>% 
  filter(!is.na(LatitudeWgs84)) %>%
...
mutate(
  ...
  group_by(StationName, Datetime, SampleCode, Date, Parameter, Sign, RptLimit) %>%
  slice(1) %>%
  ungroup() %>%
  pivot_wider(
    names_from = Parameter,
    values_from = c(Value, Sign),
    id_cols = c(StationName, Datetime, SampleCode, Date),
    names_glue = "{Parameter}_{.value}"
  ) %>%
  rename_with(~str_remove(.x, "_Value"), ends_with("_Value")) %>%
  # Fill in "=" for the NA values in the _Sign variables
  mutate(across(ends_with("_Sign"), ~ if_else(is.na(.x), "=", .x)),
         Source = "DWR_NCRO") %>%
  left_join(df_ncro_coord_c_alt, by = "StationName") %>%
  filter(!is.na(Latitude)) %>%
  # Select variable order
  select(
    Source,
