...
stations <- c("FAL", "FRK", "HLT", "HOL", "OSJ", "SJC")
...
```{r}
saveRDS(flow_clean, here(data_root,"CDEC_flow_FAL_HLT_HOL_OSJ_SJC_2015_2022.rds"))
flow_clean <- readRDS(here(data_root,"CDEC_flow_FAL_HLT_HOL_OSJ_SJC_2015_2022.rds"))
```
...
```{r}
saveRDS(vel_clean, here("analysis_2022", "data_clean", "CDEC_velocity_2015_2022.rds"))
vel_clean <- readRDS(here("analysis_2022", "data_clean", "CDEC_velocity_2015_2022.rds"))
```
...
data_FAL <- map(plist[1:7], ~ f_cleanparam(sta = "FAL", param = .x, enddate = "20220926")) %>% bind_rows()
data_OSJ <- map(plist[1:7], ~ f_cleanparam(sta = "OSJ", param = .x, enddate = "20220926")) %>% bind_rows()
data_HLT <- map(plist[-c(5:6, 8)], ~ f_cleanparam3(sta = "HLT", param = .x, enddate = "20220908")) %>% bind_rows()
data_HOL <- map(plist[1:6], ~ f_cleanparam2(sta = "HOL", param = .x, enddate = "20220823")) %>% bind_rows()
data_BLP <- map(plist[c(1, 3:8)], ~ f_cleanparam4(sta = "BLP", param = .x, startdate = "2015", enddate = "2022")) %>%
  bind_rows()
...
f_get_NWIS_vel <- function(siteNumbers=11312676, parameterCd = c('72255'), startDate = "2015-01-01", endDate = "2022-10-01", tz = "Etc/GMT+8"){
...
data_USGS <- rbind(MDM_comb, ORQ, DSJ, SJG, FAL, FAL_vel) 
```
