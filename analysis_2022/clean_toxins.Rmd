...
#quick plot
ggplot(swp, aes(x = as.factor(Date), y = value, fill = Species)) + geom_bar(stat = "identity")
```
...
ct_all <- rbind(swp, wb, ebparks, naut, usgs) %>%
  mutate(Analyte2 = case_when(grepl("MC",Analyte) |grepl("Microcys", Analyte) ~ "Microcystins",
                              grepl("Anatoxin", Analyte)| grepl("anatoxin", Analyte) ~ "Anatoxins",
                              grepl("Dolicho", Analyte) ~ "Dolichopspermum",
                              grepl("Anabaen", Analyte) ~ "Anabaenopeptin",
                              grepl("Cylindrospermopsin", Analyte) ~ "Cylindrospermopsin",
                              grepl("Oscillamide", Analyte) ~ "Oscillamide",
                             TRUE ~ Analyte),
...
```{r}
ct_all2 = mutate(ct_all, Advisory = case_when(Analyte2 == "Microcystins" & Result > 0.8 & Result < 6 ~ "Caution",
...
EDSM_Strata <- st_transform(deltamapr::R_EDSM_Strata_19P3, crs = 4269)
```
