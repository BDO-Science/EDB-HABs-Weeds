...
#2022 data for all regions and common Delta
...
vegarea <- read_csv("./analysis_2022/data_clean/ucd_imagery_regional_area_estimates_2022_QAQC.csv")

#######################################################
# KKA 5/22/23: Didn't do water quality analysis/graphs
#######################################################

#read in df that matches regions and discrete water quality stations
stm <- read_csv("./analysis_2022/hyperspect_veg_2022/water_quality_discrete_stations_2022.csv")
...
wqd <- read_csv("./outputs/discrete_wq&outflow_data_summary.csv") %>%
  clean_names() %>%
  rename(ph=p_h)

#read in Franks Tract fluridone application data (2006-2021)
herb <- read_csv("./data/franks_tract_fluridone_2006-2021_summary.csv")

####################################
# KKA 5/22/23: Picked up again here
####################################
...
ggsave(plot=foc_ha, "./analysis_2022/figures/Hyperspectral_Veg_Area_TimeSeries_FocalSiteArea_2022_052623.png",type ="cairo-png",width=8, scale=0.8, height=7,units="in",dpi=300)
...
ggsave(plot=veg_perc, "./analysis_2022/figures/Hyperspectral_Veg_Area_TimeSeries_DeltaCommonArea.png",type ="cairo-png",width=8, scale=0.9, height=4.5,units="in",dpi=300)
...
chart.Correlation(bb22[2:10])

#6 make comparisons among sites within land types-----------

#need to format the data wider
veg_corr <- vegarea_sub %>%
  select(site, year,veg_type,area_prop_h) %>%
  pivot_wider(id_cols = c(site,year), names_from = veg_type, values_from = area_prop_h) %>%
  pivot_wider(id_cols = c(year), names_from = site, values_from = c(sav,fav)) %>%
  #rename columns
  rename(
...
fb<-ggplot(veg_corr, aes(x=sav_prop_ft, y= sav_prop_bb))+
  ...
  ylab("Big Break SAV ") +
  coord_cartesian(xlim = c(0,0.73), ylim = c(0, 0.8))

fb_mod <- cor.test(veg_corr$sav_prop_ft,veg_corr$sav_prop_bb)
#correlation is significant
#p-value = 0.00594; cor= 0.6732489
...
#correlation is significant
#p-value = 4.086e-08; cor=0.9530261
...
#correlation is significant <-- insignificant?
#p-value = 0.2976; cor=0.3130688

sfigure <- ggarrange(fb,fc,
                     labels = c("A", "B"),
                     ncol = 2, nrow = 1)
ggsave(plot=sfigure, "./analysis_2022/figures/Hyperspectral_SAV_AreaCorrPanel_FT_CC_BB_2022_052623.png",type ="cairo-png",width=8, height=4.5,units="in",dpi=300)

ffigure <- ggarrange(fbf,fcf,
                     labels = c("A", "B"),
                     ncol = 2, nrow = 1)
ggsave(plot=ffigure, "./analysis_2022/figures/Hyperspectral_FAV_AreaCorrPanel_FT_CC_BB_2022_052623.png",type ="cairo-png",width=8, height=4.5,units="in",dpi=300)

############################
# KKA 5/22/23: Stopped here
############################
...
ggsave(plot=flur, "./analysis_2022/hyperspect_veg_2022/Hyperspectral_FT_SAV_Area_v_Fluridone.png",type ="cairo-png",width=8, scale=0.9, height=4.5,units="in",dpi=300)
...
vaw <- vegarea_sub %>%
  ...
  pivot_wider(id_cols = c(site,year), names_from = veg_type, values_from = area_ha)
...
bbwr <- wrv %>%
  filter(site=="Big Break") %>%
  select(site,year,out_mean,conductivity,sav,fav)
...
f_corr_matrix <- round(cor(dft[4:12],use="pairwise.complete.obs"),3)
b_corr_matrix <- round(cor(dbb[c(4:9,11)],use="pairwise.complete.obs"),3)

# Computing correlation matrix with p-values
f_corrp_matrix <- round(cor_pmat(dft[4:12],use="pairwise.complete.obs"),3)
b_corrp_matrix <- round(cor_pmat(dbb[c(4:9,11)],use="pairwise.complete.obs"),3)
