...
ptox_edb <- read_excel(here(root, "data_raw", "ToxinResults_Barriersampling_EH.xlsx"), sheet = "PTOX results")
...
```{r}
ct_all <- rbind(swp, wb, ebparks, naut, usgs, prop1, restore) %>%
  mutate(Analyte2 = case_when(grepl("MC",Analyte) |grepl("Microcys", Analyte) | grepl("D-Asp3-", Analyte)|grepl("Leu1", Analyte)|grepl("Dha-LR", Analyte)|grepl("Nodularin", Analyte) ~ "Microcystins/Nodularins",
                              ...
                              grepl("Cylindrospermopsin", Analyte)|grepl("CYN", Analyte) ~ "Cylindrospermopsins",
                             TRUE ~ Analyte),
...
ct_2021_2022 <- rbind(ct_2021, ct_2022 %>% select(-Advisory, -AnalyteOrig, -StationName, -Method))
